...
public class UserBehaviorController {
    ...
    public ResponseEntity<Map<String, Object>> recordBehavior(
        ...
        // 优先使用客户端上报的 sessionId（App 端一次会话内保持不变），
        // 没有时才回退到 HTTP session id
        String sessionId = behaviorData.get("sessionId") != null ?
                behaviorData.get("sessionId").toString() : request.getSession().getId();
